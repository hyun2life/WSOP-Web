...
- **크롤러 수집 단계 다각화**: 크롤링 범위를 `standings-only`, `profile-only`, `full` 3단계로 확장하여 실행 목적에 따라 최적화된 수행 시간을 보장합니다.
- **브랜드 필터별 독립 리포트 생성 및 분리**: Live/Stage 브랜드 비교 검증을 위한 신규 스크립트 (`crawl:brand-compare`) 가 추가되었으며, 특정 브랜드 필터 크롤링 결과 리포트와 산출물을 독립적으로 분리할 수 있습니다.
- **브랜드 필터 접미사 리포트 감지**: 대시보드의 파일 감지 정규식을 수정하여, 브랜드명이 접미사로 붙은 리포트 파일 (`*-WSOP-report-ko.html` 등) 도 대시보드 리포트 목록에 정상 노출되도록 개선했습니다.
- **Profile-Only 전용 리포트 뷰 최적화**: `profile-only` 모드 실행 시 리포트 페이지에서 불필요한 대회 결과 (Result) 상세 검증 탭 및 상세 매칭 테이블을 숨기고, 프로필 요약 및 프로필 탭 무결성 검증 뷰에만 초점을 맞추도록 UI 레이아웃을 개선했습니다.
