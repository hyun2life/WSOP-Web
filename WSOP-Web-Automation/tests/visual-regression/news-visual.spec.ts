...
test.describe('News Visual Regression', () => {
  ...
  // News 관련 컴포넌트 필터링
  const newsComponents = visualComponents.filter(
    c => c.componentKey.includes('news')
  );

  for (const compConfig of newsComponents) {
    test(`Visual Regression: ${compConfig.name}`, async ({ page }, testInfo) => {
      ...
      try {
        ...
        if (hasKnownException) {
          status = 'warning';
          console.warn(`[NewsVisual] Warning (Known Exception) for "${compConfig.name}": ${errorDetails}`);
        } else {
          status = 'failed';
          throw error;
        }
