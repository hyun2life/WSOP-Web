...
| **기본 전체 회귀** | Standard Regression | `npm run test:regression:standard` | Phase 1 ~ Phase 6 | 디폴트 회귀 검증 스위트 |
...
### 주요 파일 목록
*   **[regression-summary.md](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-summary.md)**: 사람이 즉시 판독할 수 있는 요약 리포트 (실행 시간, 상태 테이블, 결함 설명 포함).
*   **[regression-summary.html](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-summary.html)**: 영문 HTML 요약 리포트.
*   **[regression-summary-ko.html](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-summary-ko.html)**: 한글 HTML 요약 리포트.
*   **[regression-summary.json](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-summary.json)**: 전체 실행 정보 구조체.
*   **[release-gate-result.json](../WSOP-Web-Automation/artifacts/full-regression/latest/release-gate-result.json)**: 릴리즈 게이트 적격 여부 판정 데이터 (`PASSED`, `FAILED`, `REQUIRES_REVIEW`)와 CI 판정 필드(`ci.shouldFailBuild`, `ci.exitCode`).
*   **[regression-failures.json](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-failures.json)**: 실패한 단계들의 exit code 및 에러 로그 목록.
*   **[regression-warnings.json](../WSOP-Web-Automation/artifacts/full-regression/latest/regression-warnings.json)**: 허용된 예외성 Warning 정보 목록.
...
3.  **🔴 FAILED (Exit Code: 1)**
    *   **기준**: 필수(Required) 단계(예: Phase 1, 2, 3, 5, 6) 중 하나라도 실패한 경우.
...
4.  **Actual Product Issue (기능 리그레션)**
    *   **현상**: Phase 1, 2, 3, 5, 6 등 핵심 경로 테스트의 단언문(Assertion) 실패.
