...
| `--brand`       | 스탠딩 목록 수집 시 필터링할 브랜드명 목록입니다. 파이프(`|`)로 구분하여 다중 지정할 수 있습니다. (예: `WSOP|GGPoker`) |
| `--season`      | 스탠딩 목록 수집 시 필터링할 시즌(연도) 목록입니다. 파이프(`|`)로 구분하여 다중 지정할 수 있습니다. (예: `2026|2025`) |
| `--profile-brand`| 개별 플레이어 프로필 페이지 진입 시 적용할 브랜드 필터명입니다. (예: `WSOP`, `GGPoker`) |
| `--profile-season`| 개별 플레이어 프로필 페이지 진입 시 적용할 시즌(연도) 필터입니다. (예: `2024`) |
...
- 2단계: 결함 카테고리
- 3단계: 대회명
- 4단계: 이벤트명 또는 대회 공통 메타데이터
- 5단계: 결함 상세 결과 테이블

결함 후보 목록 상단의 `결함 카테고리` 드롭다운으로 특정 결함 유형만 필터링해서 볼 수 있습니다.
